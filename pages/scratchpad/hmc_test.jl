...
scatter!(divergences_naive_param[!, "x[1]"], divergences_naive_param[!, :y], subplot = 1, markershape = :x, color = :aquamarine, opacity = 0.8)
...
mcmc_trace(DataFrame(raw_chain), np = nuts_params(DataFrame(raw_chain)))